...
int main(int argc, char ** argv) {
	...
	for( j=0; j<n; j++ ){
	   mpi_dorth_cgs2_threesynch_append1( mloc, j, Q, mloc, R+j*ldr, work, MPI_COMM_WORLD);
	}
	...
	if ( testing ){
		...
		mpi_dcheck_qq_orth( MPI_COMM_WORLD, &norm_orth, mloc, n, Q, ldq );
		if( my_rank == 0 ) printf("                %5.1e ",norm_orth);
		mpi_dcheck_qr_repres( MPI_COMM_WORLD, &norm_repres, mloc, n, A, lda, Q, ldq, R, ldr );
